export default function LockIcon() {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="w-5 h-5 flex-shrink-0"
    >
      <path
        opacity="0.4"
        d="M5 8.33317V6.6665C5 3.90817 5.83333 1.6665 10 1.6665C14.1667 1.6665 15 3.90817 15 6.6665V8.33317"
        stroke="#9CA3AF"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        opacity="0.4"
        d="M9.99984 15.4167C11.1507 15.4167 12.0832 14.4839 12.0832 13.3333C12.0832 12.1827 11.1507 11.25 9.99984 11.25C8.84924 11.25 7.9165 12.1827 7.9165 13.3333C7.9165 14.4839 8.84949 15.4167 9.99984 15.4167Z"
        stroke="#9CA3AF"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M14.1665 18.3335H5.83317C2.49984 18.3335 1.6665 17.5002 1.6665 14.1668V12.5002C1.6665 9.16683 2.49984 8.3335 5.83317 8.3335H14.1665C17.4998 8.3335 18.3332 9.16683 18.3332 12.5002V14.1668C18.3332 17.5002 17.4998 18.3335 14.1665 18.3335Z"
        stroke="#9CA3AF"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}
